...
router.post('/signup', async (req, res) => {
  try {
    ...
    db.query(checkEmailQuery, [email], async (checkEmailErr, checkEmailResult) => {
      ...
      const insertUserQuery = 'INSERT INTO user (email, username, password, createdat) VALUES (?, ?, ?, ?)';
      db.query(insertUserQuery, [email, username, hashedPassword, createdat], (insertErr, result) => {
        if (insertErr) {
...
router.put('/:id',authenticateToken, async (req, res) => {
  ...
  try {
    ...
    db.query(checkUserQuery, [userId], async (checkError, checkResults, checkFields) => {
      ...
      db.query(updateUserQuery, updateValues, (updateError, updateResults, updateFields) => {
        if (updateError) {
        ...
        db.query(getUpdatedUserQuery, [userId], (getError, getResults, getFields) => {
          ...
          return res.status(200).json({ message: 'Update user data successful', user: updatedUser });
        });
      });
    });
  } catch (error) {
